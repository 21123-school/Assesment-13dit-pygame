...
main.py
  ...
  in the main game loop there is a list of funcions that run every frame that can be hot added to so things like object spawners 
  can exist just by an append.
mapload.py
  currently used as a import in main.py it handles loading textbased items from a *.pak, loading non textbased items from a *.pak,
  and also handles building maps from its coded form to it's usable state it also returns the name of the next level. there is also
